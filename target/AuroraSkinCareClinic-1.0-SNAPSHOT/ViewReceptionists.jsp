<%-- 
    Document   : ViewReceptionist
    Created on : Nov 2, 2024, 1:03:54 PM
    Author     : suraj
--%>
...
<html>
    ...
    <body>
        ...
        <main class="main">
    <!-- Page Title -->
    <div class="page-title accent-background" style="background-image:url('img/receptionist.jpg');">
        <div class="container position-relative">
            <h1>View Receptionists</h1>
    ...
    <div class="page-content">
        <div class="container-fluid px-4">
            <h1 class="mt-4 text-center">View and Manage Receptionists</h1>
            <br>

            <form id="searchForm" method="GET" action="ReceptionistController">
            ...
            <table id="tblReceptionist" class="table table-striped">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    <%
                        ...
                        List<Receptionist> searchResults = (List<Receptionist>) request.getAttribute("receptionists");
                        List<Receptionist> receptionistsToDisplay = (searchResults != null && !searchResults.isEmpty()) ? searchResults : (List<Receptionist>) request.getAttribute("receptionists");

                        if (receptionistsToDisplay != null && !receptionistsToDisplay.isEmpty()) {
                            for (Receptionist receptionist : receptionistsToDisplay) {
                    %>
                    <tr>
                        <td><%= receptionist.getE_ID() %></td>
                        <td><%= receptionist.getE_Name() %></td>
                        <td><%= receptionist.getE_NIC() %></td>
                        <td><%= receptionist.getE_Phone_Number() %></td>
                        <td><%= receptionist.getSpokenLanguages() %></td> <!-- Assuming Languages is the attribute -->
                        <td>
                            <a href="ReceptionistController?action=edit&e_ID=<%= receptionist.getE_ID() %>" class="edit-receptionist btn btn-warning" data-id="<%= receptionist.getE_ID() %>">Edit</a>
                            <a href="ReceptionistController?action=delete&e_ID=<%= receptionist.getE_ID() %>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this receptionist?');">Delete</a>
                        </td>
                    </tr>
                    <%
                            }
