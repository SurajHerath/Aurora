...
<html>
    ...
    <div class="page-content">
        <div class="container-fluid px-4">
            <h1 class="mt-4 text-center">View and Manage Dermatologists</h1>
            <br>

            <form id="searchForm" method="GET" action="DermatologistController">
            ...
            <table id="tblDermatologist" class="table table-striped">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Name</th>
                ...
                <tbody>
                    <%
                        ...
                        List<Dermatologist> searchResults = (List<Dermatologist>) request.getAttribute("dermatologists");
                        List<Dermatologist> dermatologistsToDisplay = (searchResults != null && !searchResults.isEmpty()) ? searchResults : (List<Dermatologist>) request.getAttribute("dermatologists");

                        if (dermatologistsToDisplay != null && !dermatologistsToDisplay.isEmpty()) {
                            for (Dermatologist dermatologist : dermatologistsToDisplay) {
                    %>
                    <tr>
                        <td><%= dermatologist.getE_ID() %></td>
                        <td><%= dermatologist.getE_Name() %></td>
                        <td><%= dermatologist.getE_NIC() %></td>
                        <td><%= dermatologist.getE_Phone_Number() %></td>
                        <td><%= dermatologist.getSpecialization() %></td>
                        <td>
                            <a href="DermatologistController?action=edit&e_ID=<%= dermatologist.getE_ID() %>" class="edit-dermatologist btn btn-warning" data-id="<%= dermatologist.getE_ID() %>">Edit</a>
                            <a href="DermatologistController?action=delete&e_ID=<%= dermatologist.getE_ID() %>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this dermatologist?');">Delete</a>
                        </td>
                    </tr>
                    <%
                            }
                    ...
                    %>
                    <tr>
                        <td colspan="6">No dermatologists found.</td>
                    </tr>
                    <%
                        }
                    %>
